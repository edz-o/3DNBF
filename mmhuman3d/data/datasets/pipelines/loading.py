...
class LoadMaskFromFile(object):
    ...
    Required keys are "mask_prefix" and "img_info" (a dict that must contain the
    key "filename"). Added or updated keys are "filename", "img", "img_shape",
    "ori_shape" (same as `img_shape`) and "img_norm_cfg" (means=0 and stds=1).
    Both "img_shape" and "ori_shape" use (height, width) convention.
    ...
    def __call__(self, results):
        ...
        if results['mask_prefix'] is not None:
            filename = osp.join(results['mask_prefix'], results['mask_path'])
        else:
            filename = results['mask_path']
        ...
        results['mask'] = mask
        results['mask_shape'] = mask.shape[:2]
