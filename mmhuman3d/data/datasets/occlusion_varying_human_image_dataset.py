...
class OcclusionVaryingHumanImageDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.n_occ_levels = len(self.occ_levels)
        self.map_occ_level_idx = { l : i for i, l in enumerate(occ_levels)}
        self.num_data = len(self.dataset) * len(self.occ_levels)
    ...
    def __getitem__(self, idx):
        idx_org = idx // self.n_occ_levels
        info = self.dataset.prepare_raw_data(self.dataset.get_indices()[idx_org])
        info['idx'] = idx
        if self.texture_files is not None:
            info['texture_file'] = self.texture_files[self.dataset.get_indices()[idx_org]]
        
        # info['texture_file'] = "data/dtd/images/spiralled/spiralled_0014.jpg"
        info['texture_file'] = 'data/dtd/images/frilly/frilly_0029.jpg'
        occ_level = self.occ_levels[idx % self.n_occ_levels]
        info['occ_size'] = occ_level
        ...
        if self.pred_initialization is not None:
            info['poses_init'] = np.array(self.pred_initialization['poses'])[idx] \
                            if 'poses' in self.pred_initialization else \
                                np.concatenate((np.array([[1, 0, 0], [0, -1, 0], [0, 0, -1]])[None], np.repeat(np.eye(3)[None], 23, axis=0)), axis=0)
            info['betas_init'] = np.array(self.pred_initialization['betas'])[idx] \
                            if 'betas' in self.pred_initialization else np.zeros(10)
            info['cameras_init'] = np.array(self.pred_initialization['cameras'])[idx] \
                            if 'cameras' in self.pred_initialization else np.array([1., 0., 0.])
            info['keypoints3d_init'] = np.array(self.pred_initialization['keypoints'])[idx] \
                            if 'keypoints' in self.pred_initialization else np.zeros((self.num_keypoints, 3))
    ...
    def evaluate(self,
        ...
        for _metric in metrics:
            if _metric == 'mpjpe':
                ...
                _nv_tuples, occ_info_argmax, error_elementwise = self._report_mpjpe(res, metric='pa-mpjpe')
            elif _metric == '3dpck':
                ...
                _nv_tuples, occ_info_argmax, error_elementwise = self._report_2d_pckh(res)
            else:
                raise NotImplementedError
            name_value_tuples.extend(_nv_tuples)
            mmcv.dump(occ_info_argmax, os.path.join(res_folder, f'result_occ_info_{_metric}.json'))
    ...
    def _report_mpjpe(self, res_file, metric='mpjpe'):
        ...
        occ_idx_argmax = np.argmax(error_elementwise.reshape(-1, self.n_occ_levels), axis=1)
        ...
        for i, occ_idx in enumerate(occ_idx_argmax):
            for k, v in res_file['occ_info'].items():
                occ_info_argmax.setdefault(k, []).append(v[i * self.n_occ_levels + occ_idx])
    ...
    def _report_2d_pckh(self, res_file):
        ...
        occ_idx_argmax = np.argmin(pckh_elementwise.reshape(-1, self.n_occ_levels), axis=1)
        occ_info_argmax = dict(occ_idx_argmax=occ_idx_argmax)
        for i, occ_idx in enumerate(occ_idx_argmax):
            for k, v in res_file['occ_info'].items():
                occ_info_argmax.setdefault(k, []).append(v[i * self.n_occ_levels + occ_idx])
    ...
    def _parse_result(self, res_file, parse_fn):
        ...
        gt_keypoints = []
        gt_keypoints_mask = []
        res_files = split_dict(res_file, self.n_occ_levels)
        for occ_idx in range(self.n_occ_levels):
