...
from mmhuman3d.utils.image_utils import generate_part_labels, get_vert_to_part
...
class BodyModelEstimator(BaseArchitecture, metaclass=ABCMeta):
    ...
    def __init__(
        ...
        if self.loss_segm_mask is not None:
            ...
            self.neural_renderer_pred = build_neural_renderer(
                hparams.MODEL.RENDERER_PRED
            )
    ...
    def train_step(self, data_batch, optimizer, **kwargs):
        ...
        if self.backbone is not None:
            img = data_batch['img']
            from mmhuman3d.models.backbones import ViT

            if isinstance(self.backbone, ViT) or isinstance(self.backbone.module, ViT):
                features = self.backbone(img[:, :, :, 32:-32])
            else:
    ...
    def run_registration(
        ...
        pred_cam_t = (
            predictions['pred_cam'].detach().clone()
        ...
        new_opt_joint_loss = (
            registrant_output['keypoint2d_loss'].sum(dim=-1).sum(dim=-1)
        )
    ...
    def run_registration_test(
        ...
        # is less than the current one
        update = new_opt_joint_loss < opt_joint_loss
        logger.debug(f"update: {update.sum()}/{update.shape[0]}")
    ...
    def run_openpose_estimator(self, imgs, img_metas, **kwargs):
        openpose_dets = []
        if kwargs['keypoints2d'].shape[1] == 49:
            gt_keypoints_batch, _ = convert_kps(
                kwargs['keypoints2d'].cpu().numpy(),
                src='smpl_49',
                dst='h36m',
            )
        else:
        ...
        for img, img_meta, gt_keypoints in zip(
            ...
            if openpose_keypoints.any():
                ...
                joint_dists = np.nanmean(
                    np.linalg.norm(
                        openpose_keypoints[
                            :, list(constants.LSP_TO_OPENPOSE.values()), :2
                        ]
                        - gt_keypoints[None, :, :2],
                        axis=2,
                    ),
...
class ImageBodyModelEstimator(BodyModelEstimator):
    ...
    def forward_test(self, img: torch.Tensor, img_metas: dict, **kwargs):
        ...
        if self.backbone is not None:
            ...
            if isinstance(self.backbone, ViT) or (
                hasattr(self.backbone, 'module')
                and isinstance(self.backbone.module, ViT)
            ):
                features = self.backbone(img[:, :, :, 32:-32])
            else:
                features = self.backbone(img)
        ...
        if (
            ...
            predictions['pred_cam'][:, 1:] = (
                predictions['pred_cam'][:, 1:] / predictions['pred_cam'][:, 0:1]
            )
        ...
        if self.registrant is not None:
            ...
            ret = self.run_registration_test(
                predictions,
                kwargs,
                focal_length=self.hparams.DATASET.FOCAL_LENGTH,
                img_res=self.hparams.DATASET.IMG_RES,
        ...
        if self.hparams.get('return_vertices', False):
        ...
        if 'pred_segm_mask' in predictions and self.hparams.save_partseg:
            # all_preds['pred_segm_mask'] = predictions['pred_segm_mask'].detach().cpu().numpy()
            os.makedirs(
                os.path.join(self.hparams.work_dir, 'result_pred_segm_mask'),
                exist_ok=True,
            )
            for j, mask in enumerate(
                predictions['pred_segm_mask'].detach().cpu().numpy()
            ):
                idx = int(kwargs['idx'][j].item())
                dst = os.path.join(
                    self.hparams.work_dir, 'result_pred_segm_mask', f'{idx}.npy'
                )
                np.save(dst, mask)

        meta_info = {}
        if 'keypoints2d' in kwargs:
