...
from ..registrants import MonocularTotalCaptureFitting
...
class POFEstimatorModule(BaseArchitecture, metaclass=ABCMeta):
    ...
    def __init__(self,
                 ...
                 loss_pof: Optional[Union[dict, None]] = None,
                 init_cfg: Optional[Union[list, dict, None]] = None,
                 hparams: Optional[Union[dict, None]] = None):
        super(POFEstimatorModule, self).__init__(init_cfg)
        ...
        self.loss_pof = build_loss(loss_pof)

        set_requires_grad(self.body_model_train, False)
        set_requires_grad(self.body_model_test, False)

        self.neural_mesh_model = build_neural_mesh_model(hparams.MODEL.RENDERER_GT)
        ...
        if registrant is not None:
            self.fits = 'registration'
            self.fits_dict = FitsDict(fits='static')
            if isinstance(self.registrant, MonocularTotalCaptureFitting):
    ...
    def compute_pof_loss(self, pof_pred, pof_gt):
        """
            pof_pred: N, 3, res/4. res/4
        """
        loss_pof = self.loss_pof(pof_pred, pof_gt)
        return loss_pof
    ...
    def compute_losses(self, predictions: dict, targets: dict):
        ...
        pof_pred = predictions['coke_features']
        ...
        if self.hparams.renderer_type == 'VoGE':
            ...
            self.neural_mesh_model.rasterizer.cameras = new_cam
            
            with torch.no_grad():

                vert_pof = get_vert_pof(gt_keypoints3d_smpl[:, :25])
                self.neural_mesh_model.set_features(vert_pof)
                # import ipdb; ipdb.set_trace()
                ras_out = self.neural_mesh_model(gt_verts_down, 
                self.faces_down.expand(gt_verts_down.shape[0], -1, -1), 
                    )

                mask = ras_out['mask'] # B, 1, H, W
                # proj_segm_mask = ras_out['pixel_part']
                # proj_segm_mask = (proj_segm_mask.argmax(axis=1).detach() * mask).long()
                pof_gt = ras_out['projected_map'] * mask
        ...
        # import ipdb; ipdb.set_trace()
        pof_pred = F.normalize(pof_pred+1e-8, dim=1, p=2)
        pof_pred = pof_pred * mask
        if torch.isnan(pof_pred).any():
            import ipdb; ipdb.set_trace()
        if torch.isnan(pof_gt).any():
            import ipdb; ipdb.set_trace()
        
        losses['loss_pof'] = self.compute_pof_loss(pof_pred, pof_gt)

        return losses

    def get_gt_smpl(self, targets):
        ...
        return gt_pose, gt_betas, gt_cam_t        
    ...
    def run_registration_test(
        ...
        # gt_pose, gt_betas, gt_cam_t = self.get_gt_smpl(targets)
        # # # # import ipdb; ipdb.set_trace()
        # # with torch.no_grad():
