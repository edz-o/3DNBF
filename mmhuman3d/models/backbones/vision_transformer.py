...
from mmhuman3d.utils.ibot_utils import trunc_normal_
...
class FeedForwardShrink(nn.Module):
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., return_x_only=False):
        super().__init__()
    ...
    def forward(self, x, return_key=False):
        B, N, C = x.shape
        # B, N, C -> B, N, 3, heads, C // heads -> 3, B, heads, N, C // heads
        ...
        # B, heads, N, C // heads -> B, N, heads, C // heads -> B, N, C
...
class Block(nn.Module):
    ...
    def forward(self, x, return_attention=False, return_key=False):
        if return_key:
            y, attn, key = self.attn(self.norm1(x), return_key=True)
        else:
...
class VisionTransformer(BaseModule):
    """ Vision Transformer """
    def __init__(self, img_size=[224], patch_size=16, stride=None, in_chans=3, num_classes=0, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6), return_all_tokens=False, 
                 init_values=0, use_mean_pooling=False, masked_im_modeling=False, init_cfg=None, pca=None):
    ...
    def interpolate_pos_encoding(self, x, w, h):
        ...
        dim = x.shape[-1]
        if self.patch_embed.stride is not None:
            ...
            patch_pos_embed = nn.functional.interpolate(patch_pos_embed, size=(w0, h0), mode='bicubic')
            patch_pos_embed = patch_pos_embed.permute(0, 2, 3, 1).reshape(1, w0 * h0, dim)
        else:
    ...
    def forward(self, x, return_all_tokens=None, mask=None, get_vit_features=False, layer_idx=-1):
        if get_vit_features:
            return self.get_vit_features(x, layer_idx=layer_idx)
        # mim
    ...
    def get_vit_features(self, x, layer_idx=-1):
        x = self.prepare_tokens(x)
        for i, blk in enumerate(self.blocks):
            # if i < len(self.blocks) - 1:
            if i < layer_idx % len(self.blocks):
                x = blk(x)
            else:
                # return attention of the last block
                x = blk(x, return_key=True)[:, :, 1:, :]
                B, heads, hw, c = x.shape
                h = w = int(hw**0.5)
                x = x.permute(0, 1, 3, 2).reshape(B, heads*c, h, w)
