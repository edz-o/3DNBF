...
class PareHeadwCoKeNeMoAttn(BaseModule):
    def __init__(
            self,
            num_joints,
            num_input_features,
        ...
        super(PareHeadwCoKeNeMoAttn, self).__init__()
        ...
        if backbone.startswith('hrnet'):
            ...
            #     num_deconv_kernels,
            # )
            # Use U-net like architecture
            self.upsample_layers = build_head(coke_cfg)
        ...
        if self.use_soft_attention:
            ...
            self.keypoint_final_layer = nn.Conv2d(
                in_channels=n_vert+1, #num_deconv_filters[-1],
            ...
            self.smpl_proj_layer = nn.Conv2d(
                in_channels=n_vert*n_orient+1,
                out_channels=num_deconv_filters[-1],
                kernel_size=1,
                stride=1,
                padding=0,
            )
    ...
    def forward(self, features, feature_banks=None, gt_segm=None):
        ...
        output_ = self._get_2d_branch_feats(features)
        part_feats = output_['features']

        if feature_banks is not None:
            # feature_banks[0] n_orients, V, C
            # feature_banks[1] 1, C
            n_orient, V, C = feature_banks[0].shape
            # rand_features = feature_banks[0][:, torch.randperm(V)]
            assert n_orient == self.n_orient
            N, _, H, W = part_feats.shape
            # feature_banks_all = torch.cat((rand_features.view(-1, C), feature_banks[1]), dim=0)
            feature_banks_all = torch.cat((feature_banks[0].view(-1, C), feature_banks[1]), dim=0)
            smpl_feats = F.conv2d(F.normalize(output_['coke_features'], dim=1),
                                    feature_banks_all.unsqueeze(-1).unsqueeze(-1), 
                                   ) # N, n_orient*V+1, H, W
            part_feats = torch.cat((smpl_feats[:, :-1, :, :].view(N, n_orient, V, H, W).max(1)[0], # N, V, H, W
                                    smpl_feats[:, -1:, :, :]), dim=1) # N, V+1, H, W

        output['coke_features'] = output_['coke_features']
        # output.update(dict(coke_features=output['coke_features']))
        ############## GET PART ATTENTION MAP ##############
        part_attention = self._get_part_attention_map(part_feats, output)

        ############## 3D SMPL BRANCH FEATURES ##############
        smpl_feats = self.smpl_proj_layer(smpl_feats)
        ...
        pred_rotmat = rot6d_to_rotmat(pred_pose.contiguous()).reshape(batch_size, 24, 3, 3)
    ...
    def _get_2d_branch_feats(self, features):
        ...
        outputs = self.upsample_layers(features)
        # if self.use_branch_nonlocal:
