...
class JointPriorLoss(nn.Module):
    ...
    def forward(self,
        ...
        if self.use_full_body:
            ...
            #                     1)**2
            B = body_pose.shape[0]
            pose_normalized = F.normalize(body_pose.clone().reshape(B, -1, 3), p=2, dim=2).view(B, -1)
            # We subtract 3 because pose does not include the global rotation of the mode
            joint_prior_loss = torch.exp(-torch.abs(pose_normalized[:, [55-3, 58-3, 12-3, 15-3]])) ** 2
...
@LOSSES.register_module()
class VAEPosePrior(nn.Module):
    def __init__(self, prior_folder='data',
                 reduction=None,
                 loss_weight=1.0,
                 fit_z=False,
                 device='cuda'):
        super(VAEPosePrior, self).__init__()
        self.reduction = reduction
        self.loss_weight = loss_weight
        self.fit_z = fit_z
        self.device = device

        vposer_ckpt = os.path.expandvars(os.path.join(prior_folder, 'vposer_v1_0/'))
        vposer, _ = load_vposer(vposer_ckpt, vp_model='snapshot')
        vposer.to(device)
        vposer.eval()
        self.vposer = vposer

    def forward(self, body_pose, loss_weight_override=None,
                reduction_override=None):
        """
        Args:
        ...
        if self.fit_z:
            # body_pose is z
            pose_prior_loss = body_pose.pow(2).sum(dim=1)
        else:
            pose_prior_loss = self.vposer.encode(body_pose[:, :-6].float()).mean[:, :32].pow(2).sum(dim=1)
        pose_prior_loss = loss_weight * pose_prior_loss
        if reduction == 'mean':
            pose_prior_loss = pose_prior_loss.mean()
        elif reduction == 'sum':
