...
import torch
from smplx import SMPLXLayer as _SMPLX
...
class SMPLX(_SMPLX):
    ...
    def __init__(self,
        ...
        if smplx2smpl is not None:
            self.smplx2smpl = pkl.load(open(smplx2smpl, 'rb'))
    ...
    def forward(self,
        ...
        if not hasattr(self, 'joints_regressor'):
            ...
            if self.smplx2smpl is not None:
                smplx_output.vertices = torch.matmul(
                    self.smplx2smpl.repeat(batch_size, 1, 1).to(smplx_output.vertices.device)
