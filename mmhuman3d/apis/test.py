...
def collect_results_cpu(result_part, size, tmpdir=None):
    ...
    if rank != 0:
        ...
        for res in zip(*part_list):
            block = list(res)
            ordered_results.extend(block)
            cur_len += len(block)
            i_block += 1
        for idx in range(total_length-cur_len):
            ordered_results.append(part_list[idx][i_block])

        # the dataloader may pad some samples 
        # BUG (here size is not batched) 
...
def collect_results_gpu(result_part, size):
    ...
    if rank == 0:
        ...
        # the dataloader may pad some samples BUG (here size is not batched)
        ordered_results = ordered_results[:size]
        return ordered_results
