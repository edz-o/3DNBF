...
def keypoint_mpjpe(pred, gt, mask, alignment='none', return_elementwise=False):
    ...
    if alignment == 'none':
        ...
        # ])
        # Do not use invalid points for alignment
        for pred_i, gt_i, mask_i in zip(pred, gt, mask):
            if mask_i.any():
                pred_i[mask_i] = compute_similarity_transform(pred_i[mask_i], gt_i[mask_i])
    ...
    error = np.nanmean(np.linalg.norm(pred - gt, ord=2, axis=-1)[mask])
    if return_elementwise:
        error_elementwise = (np.linalg.norm(pred - gt, ord=2, axis=-1) * mask).sum(axis=1) / (mask.sum(axis=1)+1e-8)
        return error, error_elementwise
    return error
...
def keypoint_2d_pckh(pred_keypoints2d, gt_keypoints2d, gt_keypoints2d_mask, 
        thr=0.5, thr_dist_ids=(12, 13), return_elementwise=False):
    # [n, k]
    gt_keypoints2d = gt_keypoints2d[:, :, :2]
    distance = np.linalg.norm(pred_keypoints2d - gt_keypoints2d, axis=2)

    # cal thr distance
    thr_dist = thr * np.linalg.norm(
        gt_keypoints2d[:, thr_dist_ids[0], :] - gt_keypoints2d[:, thr_dist_ids[1], :], axis=-1, keepdims=False)
    thr_dist = thr_dist.reshape(-1, 1)

    # [n, k]
    correct_keypoints = (distance <= thr_dist)

    # [k]
    correct_keypoints = np.sum(gt_keypoints2d_mask * correct_keypoints, axis=0)

    # [k]
    visible_keypoints = np.sum(gt_keypoints2d_mask, axis=0)

    pckh = 100 * np.nanmean((correct_keypoints[visible_keypoints>0]/(visible_keypoints[visible_keypoints>0])))

    if return_elementwise:
        #[n]
        pckh_elementwise = (gt_keypoints2d_mask * correct_keypoints).sum(axis=1) \
                            / (gt_keypoints2d_mask.sum(axis=1)+1e-8) 
        return pckh, pckh_elementwise
    return pckh

def keypoint_2d_pck(pred_keypoints2d, gt_keypoints2d, gt_keypoints2d_mask, 
        thr=0.05, return_elementwise=False):
    # [n, k]
    gt_keypoints2d = gt_keypoints2d[:, :, :2]
    distance = np.linalg.norm(pred_keypoints2d - gt_keypoints2d, axis=2)

    # [n, k]
    correct_keypoints = (distance <= thr)
