...
def rot6d_to_rotmat_4dhuman(x: torch.Tensor) -> torch.Tensor:
    """
    Convert 6D rotation representation to 3x3 rotation matrix.
...
def estimate_translation(S, joints_2d, focal_length=5000., img_size=224., use_all_joints=False, rotation=None, use_openpose_joints=False):
...
def convert_perspective_to_weak_perspective(
    ...
    # Convert camera translation [tx, ty, tz] to Weak Perspective Camera [s, tx, ty]
    # in 3D given the bounding box size
    # if isinstance(focal_length, torch.Tensor):
    #     focal_length = focal_length[:, 0]
    # TODO support non-square images
...
def rotate_aroundy(global_rotation, angle):
    """
    Args:
        global_rotation torch.Tensor: (B, 3) axis-angle
        angle (int):in degree
    Returns:
        torch.Tensor (B, 3)
    """
    aa = torch.tensor([[0, np.radians(angle), 0]]).type_as(global_rotation)
    rotmat = axis_angle_to_matrix(aa).expand(global_rotation.shape[0], -1, -1)
    with torch.no_grad():
        return matrix_to_axis_angle(torch.bmm(axis_angle_to_matrix(global_rotation),
                    rotmat)).flatten(1)