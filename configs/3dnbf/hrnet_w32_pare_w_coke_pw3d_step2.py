...
load_from = 'exp/3dnbf_pare_kp3d_accu_2dinit_hrnetw32_new/epoch_100.pth'
...
# evaluate
evaluation = dict(
    metric=['pa-mpjpe', 'mpjpe'], eval_saved_results=False, save_best='MPJPE'
)
...
registrant = dict(
    ...
    optimizer=dict(type='Adam', lr=0.002, betas=(0.9, 0.99)),
...
model = dict(
    ...
    loss_keypoints2d=dict(
        type='MSELoss', loss_weight=2.5 * loss_weight
    ),  # 2.5*loss_weight
...
elif test_data in ['briar', 'latino']:
    test_cfg = dict(
        type=dataset_type,
        body_model=dict(
            type='GenderedSMPL',
...
data = dict(
    ...
    train=dict(
        ...
        configs=[
            ...
            #     pipeline=train_pipeline,
            #     convention='smpl_49',
            #     ann_file='pw3d_train_w_kp2d_correct.npz',
            # ),
        ],
