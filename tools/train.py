...
def main():
    ...
    if rank == 0:
        mean = model.feature_bank.average_meter.get_mean().cpu().numpy()
        var = model.feature_bank.average_meter.get_sample_var().cpu().numpy()
        count = model.feature_bank.average_meter.get_count().cpu().numpy()
        import numpy as np

        np.savez(
            osp.join(cfg.work_dir, 'mean_var.npz'), mean=mean, var=var, count=count
        )
