...
DATASET_CONFIGS = dict(
    ...
    pw3d=dict(type='Pw3dConverter', modes=['train'], device='cuda'), # 'test',
    pw3d_occ=dict(type='Pw3dOccConverter', modes=['train'], device='cuda', prefix='pw3d'),
    ...
    h36m_p1=dict(
        ...
        prefix='h36m'),
    h36m_p2=dict(
        type='H36mConverter', modes=['valid'], protocol=2, prefix='h36m', extract_img=True),
    # mpi_inf_3dhp=dict(type='MpiInf3dhpConverter', modes=['train', 'test'], extract_img=True),
    mpi_inf_3dhp=dict(type='MpiInf3dhpConverter', modes=['train'], extract_img=True),
    ...
    spin=dict(
        type='SpinConverter',
        modes=['mpi_inf_3dhp'],#['coco_2014', 'lsp', 'mpii', 'mpi_inf_3dhp', 'lspet'],
        prefix='spin_data'),
    h36m_spin=dict(
    ...
    threedoh=dict(
        type='ThreeDOHConverter', modes=['train', 'test'], device='cuda', prefix='3doh50k'),
)
